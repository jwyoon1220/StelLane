...
class VulkanTexture private constructor(
    ...
    companion object {
        private const val FORMAT = VK_FORMAT_R8G8B8A8_UNORM
        ...
        fun createFromRgba(
            ...
            submitOneTimeCommands(device, commandPool, queue) { cmd ->
                transitionLayout(cmd, image.image, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL)
                copyBufferToImage(cmd, staging.buffer, image.image, width, height)
                transitionLayout(cmd, image.image, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL)
            }

            allocator.destroyBuffer(staging)

            val view = createImageView(device, image.image)
            return VulkanTexture(image, view, width, height)
        }

        private fun createImageView(device: VkDevice, image: Long): Long = stackPush().use { stack ->
        ...
        private fun transitionLayout(cmd: org.lwjgl.vulkan.VkCommandBuffer, image: Long, oldLayout: Int, newLayout: Int) =
            stackPush().use { stack ->
        ...
        private fun copyBufferToImage(cmd: org.lwjgl.vulkan.VkCommandBuffer, buffer: Long, image: Long, width: Int, height: Int) =
            stackPush().use { stack ->
